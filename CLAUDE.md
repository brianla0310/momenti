...
Immediate foundation before the photo work: a small **JSX lint + test** setup (ESLint currently only checks `.ts`/`.tsx`, so `.jsx` is unlinted, and there is no test runner yet) — then **photo upload**.
