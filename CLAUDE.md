...
15. Local-date calendar: month/year, today, days-in-month, and Monday-first offset are derived from the browser's **current local date** (no more hardcoded July 2026 / day 15). Pure helpers in `apps/web/src/calendar/dateUtils.js` (injectable base date); page keys stay `YYYY-MM` / `YYYY-MM-DD`. Legacy `2026-07` data is preserved as-is — migrations never move it to the current month.
16. Past-month/week navigation: a `visibleMonth` state (year + 0-based monthIndex) lets the diary revisit any saved past month, and a `weekAnchor` **local date descriptor** (`{ year, monthIndex, day }`) drives the week strip. The header arrows are **view-aware** — in month view they step months (next disabled on the current month); in week view they step **±7 real days** (next disabled on the week containing today, so year/month boundaries page naturally). **`Oggi`** returns to today (this month / this week) and shows whenever the view is off today's month or week; a **month/year picker sheet** (`components/MonthPickerSheet.jsx`, `role="dialog"` + Escape/backdrop close + focus return; `components/MonthNavigation.jsx` in the header). The monthly spread and day pages derive keys from `visibleMonth`; each week cell carries its **own** real `YYYY-MM-DD` key (cross-month days keep their real month — never combined with the visible month) via `getLocalWeekDays`. Navigation-only: `visibleMonth`/`weekAnchor` are **never persisted** (a refresh starts on the real current month/week) and switching **never moves/copies/migrates** stored data. Invariants: **past months/weeks are read by their existing `YYYY-MM` / `YYYY-MM-DD` keys**; **future months/weeks are not visitable** and future days stay disabled (year/month/day comparison); month-grid leading blanks are inert placeholders (never date buttons); the week strip stays navigation-only. Bookshelf keeps the real current-month label. Storage schema v3, migration, and `LEGACY_MONTH_KEY` are unchanged.
17. Persistence safety hardening: loading now **classifies** the stored blob — `empty` / `ready` / `migrated` / `unsupported` / `invalid` — via a pure `apps/web/src/data/persistence.js` module (parse · minimal top-level shape validation · migration decision, no localStorage access). A schema **newer than v3**, an unknown version, corrupt JSON, or an unsafe top-level shape is **preserved untouched**: the app shows a blocking recovery notice and **never auto-overwrites** it with an empty v3 (a `canPersist` gate on the save effect — `unsupported`/`invalid` are never written back). Valid v1/v2/v3 load exactly as before (migrations, page keys, and `LEGACY_MONTH_KEY` destination unchanged). `localStorage.setItem` failures (quota / blocked storage) are caught so the app never crashes and are surfaced to the user once. **Storage schema stays v3**; the `momenti.v1` key, `LEGACY_MONTH_KEY`, and all `YYYY-MM` / `YYYY-MM-DD` page keys are unchanged. (Raw stored values are never printed to screen or console.)
18. JavaScript quality gate: ESLint now checks the real `.js` / `.jsx` application code (plus existing `.ts` / `.tsx`) with recommended JavaScript rules, Rules of Hooks, dependency warnings, and the Vite React Refresh component-export rule. Vitest provides deterministic unit coverage for the pure calendar boundary helpers, day-thumbnail selection, and persistence parse/validate/migration safety contract. Use `npm run test` for a single run and `npm run test:watch` while developing.
...
Immediate next work: **photo upload + two sticker styles** (polaroid + die-cut), with photo Blobs in IndexedDB and 1080px import resizing. The JSX lint + core-unit-test foundation is now in place.
